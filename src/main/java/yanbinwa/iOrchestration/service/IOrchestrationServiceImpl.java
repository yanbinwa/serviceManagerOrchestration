...
@ConfigurationProperties(prefix = "iOrchestration")
public class IOrchestrationServiceImpl implements IOrchestrationService
{
    
    private static final Logger logger = Logger.getLogger(IOrchestrationServiceImpl.class);
    
    @Value("${iOrchestration.dependencyProperties:}")
    ...
    @Override
    public ZNodeServiceData getRegZnodeData(String zNodeName) throws KeeperException, InterruptedException
    {
        if (zk == null)
        {
    ...
    @Override
    public boolean isRegZnodeExist(String serviceName) throws KeeperException, InterruptedException
    {
    ...
    @Override
    public boolean isDepZnodeExist(String serviceGroupName) throws KeeperException, InterruptedException
    {
    ...
    @Override
    public void createDepZnode(String serviceGroupName, ZNodeDependenceData zNodeDependenceData) throws KeeperException, InterruptedException
    {
        String path = getDepZnodePathForChildNode(serviceGroupName);
    ...
    @Override
    public void updateDepZnode(String serviceGroupName, ZNodeDependenceData zNodeDependenceData) throws KeeperException, InterruptedException
    {
        String path = getDepZnodePathForChildNode(serviceGroupName);
    ...
    @Override
    public void deleteRegZnode(String serviceName) throws InterruptedException, KeeperException
    {
        String path = getRegZnodePathForChildNode(serviceName);
    ...
    @Override
    public void createRegZnode(String serviceName, ZNodeData zNodeData) throws KeeperException, InterruptedException
    {
        String path = getRegZnodePathForChildNode(serviceName);
    ...
    @Override
    public void deleteDepZnode(String serviceName) throws InterruptedException, KeeperException
    {
    ...
    private void init()
    {
        ...
        /* Zookeeper操作相关信息 */
        regZnodePath = zNodeInfoProperties.get(IOrchestrationService.ZNODE_REGPATH);
        regZnodeChildPath = zNodeInfoProperties.get(IOrchestrationService.ZNODE_REGCHILDPATH);
        depZnodePath = zNodeInfoProperties.get(IOrchestrationService.ZNODE_DEPPATH);
        zookeeperHostport = zNodeInfoProperties.get(IOrchestrationService.ZK_HOSTPORT);
    }
    ...
    {
        String serviceGroup = serviceDataProperties.get(IOrchestrationService.SERVICE_SERVICEGROUPNAME);
        String serviceName = serviceDataProperties.get(IOrchestrationService.SERVICE_SERVICENAME);
        String ip = serviceDataProperties.get(IOrchestrationService.SERVICE_IP);
        String portStr = serviceDataProperties.get(IOrchestrationService.SERVICE_PORT);
        int port = Integer.parseInt(portStr);
        String rootUrl = serviceDataProperties.get(IOrchestrationService.SERVICE_ROOTURL);
    ...
    private void zookeeperEventHandler()
    {
        ...
        try
        {
            ...
            else if(!ZkUtil.checkZnodeExist(zk, regZnodeChildPath))
            {
                ...
                Thread.sleep(IOrchestrationService.ZKNODE_REGCHILDPATH_WAITTIME);
    ...
    private void handerZookeeperEventAsStandby()
    {
        ...
        try
        {
            ...
            {
                WatchedEvent event = zookeeperEventQueue.poll(IOrchestrationService.ZKEVENT_QUEUE_TIMEOUT, 
    ...
    private void syncWithZookeeper()
    {
        while(isRunning && zk != null)
        {
            ...
            try
            {
                Thread.sleep(IOrchestrationService.ZK_SYNC_INTERVAL);
            } 
            catch (InterruptedException e)
            {
                if(!isRunning)
